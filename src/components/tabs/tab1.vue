<template>
      ...
      <v-dialog v-model="colordialog" width="auto">
        <v-card title="颜色选择器">
          <div class="d-flex flex-column">
            <v-color-picker v-model="color[selectcolor]" v-model:mode="mode"></v-color-picker>
          </div>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn text="确认" variant="elevated" @click="colordialog = false"></v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-form>
  ...
  <v-snackbar :timeout="2000" variant="tonal" color="var(--leleo-vcard-color)" rounded="pill" v-model="snackbar">{{snackbartext}}</v-snackbar>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      configdata: config,
      snackbartext: '',
      color: { "themecolor": '', "welcometitlecolor": '' },
      brightness: 78,
      blur: 0,
